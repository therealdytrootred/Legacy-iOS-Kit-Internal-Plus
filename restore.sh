...
set_tool_paths() {
    ...
    if [[ $OSTYPE == "linux"* ]]; then
        ...
        zenity2="$(command -v zenity)"
        zenity="zenity_wrapper"
        scp2="$dir/scp"
        ssh2="$dir/ssh"
        cp $ssh2 . 2>/dev/null
...
version_update_check() {
    ...
    download_from_url "https://api.github.com/repos/therealdytrootred/Legacy-iOS-Kit-Internal-Plus/releases/latest" latest
...
device_get_info() {
    ...
    case $device_type in
        ...
        # A12 - iPhone XS/XR family
        iPhone11,2) device_model="d321";;   # iPhone XS
        iPhone11,4) device_model="d331";;   # iPhone XS Max (China Mobile)
        iPhone11,6) device_model="d331p";;  # iPhone XS Max (worldwide)
        iPhone11,8) device_model="n841";;   # iPhone XR
        # A13 - iPhone 11 family
        iPhone12,1) device_model="n104";;   # iPhone 11
        iPhone12,3) device_model="d421";;   # iPhone 11 Pro
        iPhone12,5) device_model="d431";;   # iPhone 11 Pro Max
        iPhone12,8) device_model="d79";;    # iPhone SE (2nd gen)
        # A14 - iPhone 12 family
        iPhone13,1) device_model="d52g";;   # iPhone 12 mini
        iPhone13,2) device_model="d53g";;   # iPhone 12
        iPhone13,3) device_model="d53p";;   # iPhone 12 Pro
        iPhone13,4) device_model="d54p";;   # iPhone 12 Pro Max
        # A15 - iPhone 13 family
        iPhone14,2) device_model="d63";;    # iPhone 13 Pro
        iPhone14,3) device_model="d64";;    # iPhone 13 Pro Max
        iPhone14,4) device_model="d16";;    # iPhone 13 mini
        iPhone14,5) device_model="d17";;    # iPhone 13
        iPhone14,6) device_model="d49";;    # iPhone SE (3rd gen)
        iPhone14,7) device_model="d27";;    # iPhone 14
        iPhone14,8) device_model="d28";;    # iPhone 14 Plus
        # A16 - iPhone 14 Pro family
        iPhone15,2) device_model="d73";;    # iPhone 14 Pro
        iPhone15,3) device_model="d74";;    # iPhone 14 Pro Max
        # A15 - iPhone 15 standard
        iPhone15,4) device_model="d37";;    # iPhone 15
        iPhone15,5) device_model="d38";;    # iPhone 15 Plus
        # A17 - iPhone 15 Pro family
        iPhone16,1) device_model="d83";;    # iPhone 15 Pro
        iPhone16,2) device_model="d84";;    # iPhone 15 Pro Max
        # A18 - iPhone 16 Pro family
        iPhone17,1) device_model="d93";;    # iPhone 16 Pro
        iPhone17,2) device_model="d94";;    # iPhone 16 Pro Max
        # A18 - iPhone 16 standard family
        iPhone17,3) device_model="d47";;    # iPhone 16
        iPhone17,4) device_model="d48";;    # iPhone 16 Plus
        # A16 - iPhone 16e
        iPhone17,5) device_model="v59";;    # iPhone 16e
    ...
    if (( device_proc > 10 )) && [[ -z $device_unsupported_warned ]]; then
...
device_iproxy() {
    ...
    # Kill any existing iproxy already bound to ssh_port to avoid "Address already in use" errors
    if [[ -n $iproxy_pid ]]; then
        kill $iproxy_pid 2>/dev/null
        iproxy_pid=
    fi
    kill $(lsof -ti tcp:$ssh_port 2>/dev/null) 2>/dev/null || true
    if [[ $1 == "no-logging" && $debug_mode != 1 ]]; then
        "$dir/iproxy" $ssh_port $port -s 127.0.0.1 >/dev/null 2>/dev/null &
        iproxy_pid=$!
    else
...
ipsw_preference_set() {
    ...
    if [[ -n $device_bootargs ]]; then
        log "Custom bootargs provided. Skipping verbose boot prompt."
        ipsw_verbose=0
    elif [[ $ipsw_canverbose == 1 && -z $ipsw_verbose ]]; then
...
ipsw_patch_iboot_args() {
    ...
    # Determine string based on verbose selection or custom bootargs
    local patch_str
    if [[ -n $device_bootargs ]]; then
        # Convert custom bootargs string to spaced hex representation
        patch_str=$(echo -n "$device_bootargs" | xxd -p -c 256 | sed 's/../& /g' | sed 's/ $//' | tr '[:lower:]' '[:upper:]')
        log "Applying custom bootargs hex string..."
    elif [[ $ipsw_verbose == 1 ]]; then
        patch_str="2D 76 20 61 6D 66 69 3D 30 78 66 66 20 63 73 5F 65 6E 66 6F 72 63 65 6D 65 6E 74 5F 64 69 73 61 62 6C 65 3D 31"
    else
        patch_str="61 6D 66 69 3D 30 78 66 66 20 63 73 5F 65 6E 66 6F 72 63 65 6D 65 6E 74 5F 64 69 73 61 62 6C 65 3D 31"
    fi
    ...
    # Use -0 to unconditionally replace and store
    zip -0 temp.ipsw "$dir_struct/$iboot_filename" >/dev/null
...
ipsw_prepare() {
    ...
    if [[ -n $custom_kernelcache_path && $device_target_powder == 1 && $ipsw_skipped_creation != 1 ]]; then
        ipsw_process_custom_kernelcache
    fi

    # Setup.app Deletion Hook
    if [[ $ipsw_delsetupapp == 1 && $ipsw_appleinternal != 1 && $ipsw_skipped_creation != 1 ]]; then
        ipsw_process_setupapp
    fi

    # Global AppleInternal Hook (Runs once at the end)
    if [[ $ipsw_appleinternal == 1 && $ipsw_skipped_creation != 1 ]]; then
        ipsw_process_appleinternal
    fi
}
...
device_ramdisk64() {
    ...
    if [[ $device_ramdisk_ios8 == 1 ]]; then
        ...
        sshtar="../saved/ssh_pre12.tar"
        if [[ ! -e $sshtar ]]; then
            log "Downloading ssh_pre12.tar from sshtars..."
            file_download https://raw.githubusercontent.com/lukezgd/sshtars/main/ssh_pre12.tar.gz ssh_pre12.tar.gz
            gzip -d ssh_pre12.tar.gz
            mv ssh_pre12.tar $sshtar
        fi
        cp $sshtar ssh.tar
    else
        comps+=("Trustcache")
        if [[ ! -e $sshtar.gz ]]; then
            log "Downloading ssh.tar from sshtars..."
            file_download https://raw.githubusercontent.com/lukezgd/sshtars/main/ssh.tar.gz ssh.tar.gz
    ...
    for getcomp in "${comps[@]}"; do
        ...
        # Fix double .dmg extension for RestoreRamdisk
        if [[ $getcomp == "RestoreRamdisk" && $name == *.dmg.dmg ]]; then
            name="${name%.dmg}"
        fi
        ...
        # Safely handle devices/firmwares that are not encrypted (like A11 / iOS 14+)
        local k_args=""
        if [[ -n $iv && -n $key ]]; then
            k_args="-k ${iv}${key}"
        fi
        ...
        case $getcomp in
            "iBSS" | "iBEC" )
                ...
                # $k_args must be unquoted so it expands properly or evaporates if empty
                "$dir/img4" -i $getcomp.orig -o $getcomp.dec $k_args
                mv $getcomp.orig $getcomp.orig0
                if [[ $device_ramdisk_ios8 == 1 ]]; then
                    $bspatch $getcomp.dec $getcomp.orig ../resources/sshrd/ios8/$name.patch
                else
                    if [[ $getcomp == "iBSS" ]]; then
                        "$dir/kairos" $getcomp.dec $getcomp.patched
                    else
                        local ibec_args="rd=md0 debug=0x2014e -v wdt=-1"
                        if [[ $device_proc == 7 || $device_proc == 8 ]]; then
                            ibec_args+=" nand-enable-reformat=1 -restore"
                        fi
                        "$dir/kairos" $getcomp.dec $getcomp.patched -b "$ibec_args" -n
                    fi
                    mv $getcomp.patched $getcomp.orig
                fi
            ;;
            "Kernelcache" )
                reco+="rkrn"
                if [[ $device_ramdisk_ios8 == 1 ]]; then
                    mv $getcomp.orig $getcomp.orig0
                    "$dir/img4" -i $getcomp.orig0 -o $getcomp.orig $k_args -D
                else
                    "$dir/img4" -i $getcomp.orig -o kcache.raw $k_args
                    "$dir/KPlooshFinder2" kcache.raw kcache.patched
            ...
            "DeviceTree" )
                ...
                if [[ $device_ramdisk_ios8 == 1 ]]; then
                    reco+=" -A"
                    mv $getcomp.orig $getcomp.orig0
                    "$dir/img4" -i $getcomp.orig0 -o $getcomp.orig $k_args
                fi
            ;;
    ...
    if [[ $device_ramdisk_ios8 == 1 ]]; then
        print "* Booted SSH ramdisk is based on: https://github.com/lukezgd/sshtars"
    else
...
get_gui_input() {
    ...
    local DEFAULT="$2"
    local TITLE="$3"
    osascript -e "Tell application \"System Events\" to display dialog \"$PROMPT\" default answer \"$DEFAULT\" with title \"$TITLE\" buttons {\"OK\", \"Cancel\"} default button \"OK\" with icon note" -e 'text returned of result' 2>/dev/null
}

start_mac_authentication() {
    local CONTEXT="${1:-authorize disk operations}"
    
    if $sudo -n true 2>/dev/null; then return 0; fi

    echo ""
    warn "MAC SUDO AUTHENTICATION"
    print "      (Please enter your Mac Login Password in the popup window)"
    print "      (Required to $CONTEXT)"
    
    local AUTH_PASS=$(osascript -e "Tell application \"System Events\" to display dialog \"Enter your Mac Login Password to $CONTEXT:\" default answer \"\" with title \"Mac Authentication\" with icon caution with hidden answer buttons {\"OK\", \"Cancel\"} default button \"OK\"" -e 'text returned of result' 2>/dev/null)

    if [[ -z "$AUTH_PASS" ]]; then error "Authentication Cancelled."; fi

    echo "$AUTH_PASS" | $sudo -S -v >/dev/null 2>&1
    
    if [ $? -eq 0 ]; then
        log "Mac Authentication Verified"
...
dumper_dump_nand() {
    print "* Dump NAND firmware (32-bit)"
    echo
    
    local TARGET_DIR
    input "Select where to save the NAND firmware dump in the file selection window."
    menu_zenity_check
    ...
    # Create the timestamped dump folder
    local date_str=$(date +%Y-%m-%d)
    local time_str=$(date +%H%M)
    local DUMP_DIR="$TARGET_DIR/${device_type}_${date_str}_${time_str}_NAND"
    mkdir -p "$DUMP_DIR"

    log "Starting NAND firmware dump to: $DUMP_DIR"

    if [[ $device_mode != "DFU" && $device_mode != "Recovery" ]]; then
        print "* Please put the device in DFU mode to continue."
        device_enter_mode DFU
    fi
    
    device_enter_mode pwnDFU
    
    # Send appropriate bootloaders
    if [[ $device_proc == 4 && $device_pwnrec != 1 ]]; then
        patch_ibss
        log "Sending iBSS..."
        $irecovery -f pwnediBSS.dfu
    ...
    log "Uploading payload and dumping NAND SysCfg..."
    (echo -e "/send ../resources/dumper/ibex/payload\ngo dumpnandsyscfg\n/exit") | $irecovery2 -s
    $irecovery2 -g "$DUMP_DIR/nand_syscfg"

    log "Dumping NAND Firmware..."
    (echo -e "go dumpnandfw\n/exit") | $irecovery2 -s
    $irecovery2 -g "$DUMP_DIR/nand_fw"
    
    log "Dumping NAND LLB..."
    (echo -e "go dumpnandllb\n/exit") | $irecovery2 -s
    $irecovery2 -g "$DUMP_DIR/nand_llb"
    
    log "Resetting device..."
    $irecovery -c reset
    
    if [[ -s "$DUMP_DIR/nand_syscfg" && -s "$DUMP_DIR/nand_fw" && -s "$DUMP_DIR/nand_llb" ]]; then
        log "Dump NAND firmware complete! Files saved to: $DUMP_DIR"
    else
        warn "Dump NAND firmware completed, but one or more files may be missing or empty."
    fi
    pause
}
    ...
    while [[ -z "$mode" && -z "$back" ]]; do
        ...
        if (( device_proc >= 7 )); then
            if [[ $device_mode == "Normal" && -n $device_vers_maj && $device_vers_maj -lt 10 ]]; then
                print "* Note: iOS $device_vers detected — this version uses HFS+, not APFS."
            else
                print "* Note: Devices running internal builds of iOS 10.0 or newer typically use APFS."
                print "* Older iOS versions use HFS+. You should use 'Auto-Detect Filesystem' if you are unsure."
            fi
        fi
        echo
        ...
        if [[ $device_argmode != "none" ]]; then
            if (( device_proc >= 7 )); then
                ...
                menu_items+=("Auto-Detect Filesystem")
                if (( device_proc < 11 )); then
                    if [[ $hide_apfs != 1 ]]; then
                        menu_items+=("Initiate 64-bit APFS Dump (Ramdisk)")
                    fi
                    menu_items+=("Initiate 64-bit HFS+ Dump (Ramdisk)")
                fi
                menu_items+=("Live Dumps")
            else
                menu_items+=("Dump NAND firmware")
            fi
        fi
        
        # macOS-only DMG tools — only shown for 32-bit devices or no-device mode
        if [[ $platform == "macos" ]]; then
            if (( device_proc < 7 )) || [[ $device_argmode == "none" ]]; then
                if [[ $device_argmode != "none" ]]; then
        ...
        case $selected in
            "Auto-Detect Filesystem"               ) dumper_autodetect_fs;;
            "Live Dumps"                           ) dumper_menu_live;;
            "Initiate 64-bit APFS Dump (Ramdisk)"  ) dumper_ramdisk64_dump "APFS";;
            "Initiate 64-bit HFS+ Dump (Ramdisk)"  ) dumper_ramdisk64_dump "HFS";;
            "Dump NAND firmware"                   ) dumper_dump_nand;;
...
dumper_menu_live() {
    ...
    print " > AppleInternal OS Dumper > Live Dumps"
    echo
    if [[ $hide_apfs != 1 ]]; then
        live_items+=("Initiate 64-bit APFS Dump (Live)")
    fi
    live_items+=("Initiate 64-bit HFS+ Dump (Live)")
    live_items+=("Go Back")

    input "Select an option:"
    select_option "${live_items[@]}"
    local live_selected="${live_items[$?]}"

    case $live_selected in
        "Initiate 64-bit APFS Dump (Live)" ) dumper_live64_apfs_dump;;
        "Initiate 64-bit HFS+ Dump (Live)" ) dumper_live64_hfs_dump;;
        "Go Back" ) return;;
    esac
}
...
dumper_menu_ramdisk() {
    ...
    if [[ $hide_apfs != 1 ]]; then
        rd_items+=("Initiate 64-bit APFS Dump (Ramdisk)")
    fi
    rd_items+=("Initiate 64-bit HFS+ Dump (Ramdisk)")
    rd_items+=("Go Back")

    print " > AppleInternal OS Dumper > Ramdisk Dumps"
    echo
    input "Select an option:"
    select_option "${rd_items[@]}"
    local rd_selected="${rd_items[$?]}"
...
dumper_autodetect_fs() {
    print "* Auto-Detect Filesystem"
    
    device_get_info
    if [[ $device_mode != "Normal" ]]; then
        warn "Device must be in Normal mode to auto-detect filesystem."
        pause
        return
    fi

    log "Opening SSH tunnel for filesystem detection..."
    device_iproxy no-logging
    local DUMP_IPROXY_PID=$iproxy_pid
    device_sshpass alpine

    local found=0
    local attempts=0
    log "Waiting for SSH connection..."
    while [[ $found != 1 && $attempts -lt 15 ]]; do
        if $ssh -p $ssh_port root@127.0.0.1 "echo 1" &>/dev/null; then
            found=1
        else
            sleep 2
            ((attempts++))
    ...
    if [[ $found != 1 ]]; then
        warn "Could not connect to device via SSH."
        print "* Make sure the device is jailbroken, connected, and trusted."
        kill "$DUMP_IPROXY_PID" 2>/dev/null
        pause
        return
    fi

    log "Reading /private/etc/fstab..."
    local fstab_content=$($ssh -p $ssh_port root@127.0.0.1 "cat /private/etc/fstab" 2>/dev/null)

    kill "$DUMP_IPROXY_PID" 2>/dev/null

    if echo "$fstab_content" | grep -qi "apfs"; then
        echo
        print "* Detected Filesystem: APFS"
        echo
        local opts=()
        opts+=("Live Dumps")
        if (( device_proc < 11 )); then
        ...
        case "${opts[$sel]}" in
            "Live Dumps"    ) dumper_menu_live;;
            "Ramdisk Dumps" ) dumper_menu_ramdisk;;
        esac
    elif echo "$fstab_content" | grep -qi "hfs"; then
        echo
        print "* Detected Filesystem: HFS+"
        echo
        local opts=()
        opts+=("Live Dumps")
        if (( device_proc < 11 )); then
        ...
        warn "Could not determine filesystem from fstab."
        print "* Please manually select the format from the main menu."
        pause
    fi
}

dumper_ramdisk64_dump() {
    local fs_type="$1"
    local target_part="/dev/rdisk0"
    local out_name="disk0.dmg.gz"
    local print_name="disk0"

    if [[ "$fs_type" == "HFS" ]]; then
        target_part="/dev/rdisk0s1s1"
        out_name="system.dmg.gz"
        print_name="disk0s1s1"
    fi

    print "* 64-bit $fs_type Dumper (Ramdisk)"
    echo

    # For HFS devices that support iOS 8 ramdisk, prompt which ramdisk to use
    if [[ "$fs_type" == "HFS" ]]; then
        if [[ $device_proc == 7 || ($device_proc == 8 && ($device_type == "iPhone7,"* || $device_type == "iPad5,3" || $device_type == "iPad5,4" || $device_type == "iPod7,1")) ]]; then
            input "Select SSH Ramdisk Version"
            print "* iOS 12 ramdisk is recommended for devices running iOS 9+."
            print "* iOS 8 ramdisk should be used for devices running iOS 7-8."
            echo
            local rd_ver_opts=("Use iOS 12 Ramdisk" "Use iOS 8 Ramdisk" "Go Back")
            select_option "${rd_ver_opts[@]}"
            case "${rd_ver_opts[$?]}" in
                "Use iOS 12 Ramdisk"               ) :;;
                "Use iOS 8 Ramdisk"                ) device_ramdisk_ios8=1;;
                "Go Back"                          ) return;;
            esac
        fi
    fi

    print "* Please put the device in DFU mode to boot the SSH Ramdisk."

    # Enter pwnDFU directly to bypass Normal Mode checks and run checkm8
    device_enter_mode pwnDFU
    if [[ $device_mode != "DFU" ]]; then
        warn "Device is not in pwned DFU mode. Operation cancelled."
        return
    fi

    # Temporarily set mode to dumper so it returns back to us after ramdisk boots
    local old_mode="$mode"
    mode="dumper"
    ...
    # Ask for destination
    local TARGET_DIR
    input "Select where to save the $fs_type dump in the file selection window."
    menu_zenity_check
    TARGET_DIR="$($zenity --file-selection --directory --title="Select Save Destination")"
    if [[ -z "$TARGET_DIR" ]]; then
        read -p "$(input 'Enter path to save directory (or press Enter/Return to cancel): ')" TARGET_DIR
    ...
    local date_str=$(date +%Y-%m-%d)
    local time_str=$(date +%H%M)
    local DUMP_DIR="$TARGET_DIR/${device_type}_${date_str}_${time_str}_${fs_type}"
    mkdir -p "$DUMP_DIR"

    log "Starting $fs_type Ramdisk Dump to: $DUMP_DIR"

    # $ssh is already set by device_enter_ramdisk -> device_sshpass alpine

    # Dump target partition
    log "Dumping $target_part (This will take a while)..."
    if [[ "$fs_type" == "APFS" ]]; then
        print "* Note: The size may appear to hang for a while on the same value — this is normal."
        print "* dd is working through null (empty) blocks in the disk image."
    fi
    local START_TIME=$(date +%s)
    local OUT_FILE="$DUMP_DIR/$out_name"

    $ssh -p $ssh_port root@127.0.0.1 "dd if=$target_part bs=4096" | gzip > "$OUT_FILE" &
    local DUMP_PID=$!
    ...
    while kill -0 "$DUMP_PID" 2>/dev/null; do
        if [[ -f "$OUT_FILE" ]]; then
            ...
            local mins=$(( elap / 60 ))
            local secs=$(( elap % 60 ))
            printf "\r   >> [ Dumping %s ]  Size: %-8s |  Time Elapsed: %02d:%02d " "$print_name" "$size" "$mins" "$secs"
        fi
        sleep 2
    done
    echo ""
    wait "$DUMP_PID"
    if [[ ! -s "$OUT_FILE" || $(stat -f%z "$OUT_FILE" 2>/dev/null || stat -c%s "$OUT_FILE" 2>/dev/null) -lt 524288 ]]; then
        error "Disk dump failed or produced file smaller than 512KB."
    fi
    log "Disk dump complete."

    # Dump /dev/disk1 (NVMe Firmware)
    log "Dumping NVMe Firmware (/dev/disk1)..."
    START_TIME=$(date +%s)
    OUT_FILE="$DUMP_DIR/disk1.bin"

    $ssh -p $ssh_port root@127.0.0.1 "dd if=/dev/disk1" > "$OUT_FILE" &
    DUMP_PID=$!
    ...
    while kill -0 "$DUMP_PID" 2>/dev/null; do
        if [[ -f "$OUT_FILE" ]]; then
            ...
            local mins=$(( elap / 60 ))
            local secs=$(( elap % 60 ))
            printf "\r   >>[ Dumping disk1 ]  Size: %-8s |  Time Elapsed: %02d:%02d " "$size" "$mins" "$secs"
        fi
        sleep 2
    done
    echo ""
    wait "$DUMP_PID"
    if [[ ! -s "$OUT_FILE" || $(stat -f%z "$OUT_FILE" 2>/dev/null || stat -c%s "$OUT_FILE" 2>/dev/null) -lt 524288 ]]; then
        warn "Disk dump may have failed (file smaller than 512KB)."
    fi

    # Dump /dev/disk2 (NVMe LLB)
    log "Dumping NVMe LLB (/dev/disk2)..."
    START_TIME=$(date +%s)
    OUT_FILE="$DUMP_DIR/disk2.bin"

    $ssh -p $ssh_port root@127.0.0.1 "dd if=/dev/disk2" > "$OUT_FILE" &
    DUMP_PID=$!
    ...
    while kill -0 "$DUMP_PID" 2>/dev/null; do
        if [[ -f "$OUT_FILE" ]]; then
            ...
            local mins=$(( elap / 60 ))
            local secs=$(( elap % 60 ))
            printf "\r   >>[ Dumping disk2 ]  Size: %-8s |  Time Elapsed: %02d:%02d " "$size" "$mins" "$secs"
        fi
        sleep 2
    done
    echo ""
    ...
    log "Rebooting device..."
    $ssh -p $ssh_port root@127.0.0.1 "/sbin/reboot" 2>/dev/null || true

    echo
    print "* The device is now rebooting."
    print "* Please wait for it to fully boot into normal mode."
    print "* Make sure it is plugged in and trusted."
    input "Press Enter/Return ONLY when the device is in normal mode..."
    read -s

    log "Setting up SSH tunnel for data extraction..."
    device_iproxy no-logging
    local DUMP_IPROXY_PID=$iproxy_pid
    device_sshpass alpine
    ...
    if [[ $found != 1 ]]; then
        warn "Could not connect to device via SSH in normal mode."
        print "* The $fs_type disk and NVMe dumps are saved, but /private/var could not be extracted."
        kill "$DUMP_IPROXY_PID" 2>/dev/null
        return
    fi
    ...
    log "Dumping /private/var..."
    OUT_FILE="$DUMP_DIR/data.tar.gz"

    $ssh -p $ssh_port root@127.0.0.1         "$DUMPER_TAR_CMD -cpzvf - /private/var" > "$OUT_FILE"
    dumper_cleanup_gtar
    log "/private/var dump complete."

    log "Verifying archive integrity..."
    if tar -tzf "$OUT_FILE" > /dev/null; then
        log "Archive integrity verified — data.tar.gz is complete and valid."
    else
        warn "Archive integrity check failed — data.tar.gz may be incomplete or corrupt."
    fi

    log "Fetching iOS Version info..."
    local sw_vers_out=$($ssh -p $ssh_port root@127.0.0.1 "sw_vers" 2>/dev/null)
    local prod_ver=$(echo "$sw_vers_out" | grep "ProductVersion:" | awk '{print $2}' | tr -d '\r')
    local build_ver=$(echo "$sw_vers_out" | grep "BuildVersion:" | awk '{print $2}' | tr -d '\r')

    kill "$DUMP_IPROXY_PID" 2>/dev/null

    if [[ -n "$prod_ver" && -n "$build_ver" ]]; then
        local NEW_DUMP_DIR="$TARGET_DIR/${device_type}_${prod_ver}_${build_ver}_${date_str}_${time_str}_${fs_type}"
        log "Renaming dump folder with version info..."
        mv "$DUMP_DIR" "$NEW_DUMP_DIR"
        DUMP_DIR="$NEW_DUMP_DIR"
        local dump_prefix="${device_type}_${prod_ver}_${build_ver}"
        log "Renaming dump files with version info..."
        for f in "$DUMP_DIR"/*; do
            local fname
            fname=$(basename "$f")
            mv "$f" "$DUMP_DIR/${dump_prefix}_${fname}"
        done
    fi
    log "Dump completely finished! Saved to: $DUMP_DIR"
    echo
    print "* When sharing this dump publicly:"
    print "  - Keep files exactly as dumped (do not uncompress or modify)."
    print "  - This preserves integrity for future use in Legacy iOS Kit."
    print "* Exiting Legacy iOS Kit..."
    clean_usbmuxd clean
    exit 0
}

dumper_live64_hfs_dump() {
    print "* 64-bit HFS+ Dumper (Live)"
    print "* Ensure the device is in Normal mode."
    echo

    device_get_info
    if [[ $device_mode != "Normal" ]]; then
        warn "Device is not in Normal mode. Operation cancelled."
        pause
        return
    fi
    ...
    # Ask for destination
    local TARGET_DIR
    input "Select where to save the Live HFS+ dump in the file selection window."
    menu_zenity_check
    TARGET_DIR="$($zenity --file-selection --directory --title="Select Save Destination")"
    if [[ -z "$TARGET_DIR" ]]; then
        read -p "$(input 'Enter path to save directory (or press Enter/Return to cancel): ')" TARGET_DIR
    ...
    log "Setting up SSH tunnel for live extraction..."
    device_iproxy no-logging
    local DUMP_IPROXY_PID=$iproxy_pid
    device_sshpass alpine
    ...
    local date_str=$(date +%Y-%m-%d)
    local time_str=$(date +%H%M)
    local DUMP_DIR="$TARGET_DIR/${device_type}_${date_str}_${time_str}_HFS_Live"
    mkdir -p "$DUMP_DIR"

    log "Starting Live HFS+ Dump to: $DUMP_DIR"
    ...
    $ssh -p $ssh_port root@127.0.0.1 "/sbin/mount -u -o ro /" || true

    local target_part="/dev/rdisk0s1s1"
    local out_name="system.dmg.gz"
    local print_name="disk0s1s1"
    ...
    if [[ -n "$prod_ver" && -n "$build_ver" ]]; then
        local NEW_DUMP_DIR="$TARGET_DIR/${device_type}_${prod_ver}_${build_ver}_${date_str}_${time_str}_HFS_Live"
        log "Renaming dump folder with version info..."
        mv "$DUMP_DIR" "$NEW_DUMP_DIR"
        DUMP_DIR="$NEW_DUMP_DIR"
        local dump_prefix="${device_type}_${prod_ver}_${build_ver}"
    ...
    $ssh -p $ssh_port root@127.0.0.1 "/sbin/mount -u -o rw /" || true
...
dumper_upload_gtar() {
    # Uploads GNU tar (gtar) to the device and tests if codesigning allows it to run.
    # Sets DUMPER_TAR_CMD to the gtar path on success, or "tar" (BSD fallback) otherwise.
    # Sets DUMPER_GTAR_DST to the remote path if uploaded, so callers can clean it up.
    # Returns 1 if codesigning blocked gtar AND the user chose to cancel — callers should abort.
    local gtar_src="../resources/gtar"
    DUMPER_GTAR_DST=""
    DUMPER_TAR_CMD="tar"

    if [[ ! -f "$gtar_src" ]]; then
        log "gtar not found at $gtar_src, using BSD tar."
        return 0
    fi

    log "Uploading GNU tar to device..."
    local gtar_dst="/private/var/mobile/gtar"
    $scp -P $ssh_port "$gtar_src" root@127.0.0.1:"$gtar_dst" &>/dev/null
    if [[ $? != 0 ]]; then
        warn "Could not upload gtar to device, falling back to BSD tar."
        return 0
    fi
    DUMPER_GTAR_DST="$gtar_dst"
    $ssh -p $ssh_port root@127.0.0.1 "chmod +x $gtar_dst" &>/dev/null

    # Test if gtar can execute — codesigning blocks it silently and produces no output.
    log "Testing GNU tar on device..."
    local test_out
    test_out=$($ssh -p $ssh_port root@127.0.0.1 \
        "echo test | $gtar_dst -cpf - --files-from=- 2>/dev/null | head -c 1 | wc -c" 2>/dev/null | tr -d '[:space:]')

    if [[ "$test_out" == "1" ]]; then
        log "GNU tar is working on device — using gtar for dump."
        DUMPER_TAR_CMD="$gtar_dst"
        return 0
    fi

    # gtar is blocked — clean it up and warn the user
    $ssh -p $ssh_port root@127.0.0.1 "rm -f $gtar_dst" &>/dev/null
    DUMPER_GTAR_DST=""
    echo
    warn "GNU tar is blocked by codesigning on this device."
    print "* This usually means the device does not have permissive boot arguments set."
    print "* Without GNU tar, BSD tar will be used, which may produce 'Operation not permitted'"
    print "* errors and skip files it cannot read, resulting in an incomplete dump."
    echo
    print "* To fix this, add the following to your existing boot arguments:"
    print "    amfi_get_out_of_my_way=1 amfi_allow_any_signature=1 cs_enforcement_disable=1"
    print "* If this causes a bootloop, remove those args and proceed with BSD tar only."
    echo
    select_yesno "Proceed anyway using BSD tar (dump may be incomplete)?" 0
    if [[ $? != 1 ]]; then
        log "Dump cancelled by user."
        return 1
    fi
    warn "Proceeding with BSD tar — some files may be skipped or inaccessible."
    return 0
}

dumper_cleanup_gtar() {
    if [[ -n "$DUMPER_GTAR_DST" ]]; then
        $ssh -p $ssh_port root@127.0.0.1 "rm -f $DUMPER_GTAR_DST" &>/dev/null
        DUMPER_GTAR_DST=""
    fi
}

dumper_live64_apfs_dump() {
    print "* 64-bit APFS Dumper (Live)"
    print "* Ensure the device is in Normal mode."
    echo
    if (( device_proc < 11 )); then
        print "* Note: A live APFS dump captures the root filesystem as a tar archive."
        print "* Some sandboxed or entitlement-protected files may not be accessible and will be skipped."
        print "* If you need the most complete possible dump, use the Ramdisk option instead."
        print "* If a ramdisk dump is not possible, this live dump is still very useful and complete enough for most purposes."
        echo
    fi
    ...
    # Ask for destination
    local TARGET_DIR
    input "Select where to save the Live APFS dump in the file selection window."
    menu_zenity_check
    TARGET_DIR="$($zenity --file-selection --directory --title="Select Save Destination")"
    if [[ -z "$TARGET_DIR" ]]; then
        read -p "$(input 'Enter path to save directory (or press Enter/Return to cancel): ')" TARGET_DIR
    ...
    local date_str=$(date +%Y-%m-%d)
    local time_str=$(date +%H%M)
    local DUMP_DIR="$TARGET_DIR/${device_type}_${date_str}_${time_str}_APFS_Live"
    mkdir -p "$DUMP_DIR"

    log "Starting Live APFS Dump to: $DUMP_DIR"
    ...
    log "Dumping Root Filesystem (/)..."
    local OUT_FILE="$DUMP_DIR/fs.tar.gz"

    $ssh -p $ssh_port root@127.0.0.1         "$DUMPER_TAR_CMD -cpzvf - /" > "$OUT_FILE"
    dumper_cleanup_gtar
    log "Root Filesystem dump complete."

    log "Verifying archive integrity..."
    if tar -tzf "$OUT_FILE" > /dev/null; then
        log "Archive integrity verified — fs.tar.gz is complete and valid."
    else
        warn "Archive integrity check failed — fs.tar.gz may be incomplete or corrupt."
    fi
    ...
    if [[ -n "$prod_ver" && -n "$build_ver" ]]; then
        local NEW_DUMP_DIR="$TARGET_DIR/${device_type}_${prod_ver}_${build_ver}_${date_str}_${time_str}_APFS_Live"
        log "Renaming dump folder with version info..."
        mv "$DUMP_DIR" "$NEW_DUMP_DIR"
        DUMP_DIR="$NEW_DUMP_DIR"
        local dump_prefix="${device_type}_${prod_ver}_${build_ver}"
...
dumper_convert_tar() {
    ...
    SELECTED_TAR=$(osascript -e 'POSIX path of (choose file with prompt "Select TAR or Compressed Archive:")' 2>/dev/null)
    [[ -z "$SELECTED_TAR" ]] && return

    DEST_DIR=$(osascript -e 'POSIX path of (choose folder with prompt "Select Destination:")' 2>/dev/null)
    [[ -z "$DEST_DIR" ]] && return

    log "Analyzing archive..."
    local TAR_FLAGS="-xvpf"
    ...
    start_mac_authentication "authorize archive conversion"
    ...
    hdiutil create -size "${DMG_SIZE}m" -fs "Case-sensitive HFS+" -volname "Converted" "$TEMP_DMG" -verbose || error "Failed to create DMG."

    log "Mounting DMG with permission controls..."
    hdiutil attach -owners on "$TEMP_DMG" -verbose || error "Failed to mount DMG."

    log "Extracting archive (Verbose)..."
    if ! $sudo tar $TAR_FLAGS "$SELECTED_TAR" --strip-components=$STRIP_VAL -C /Volumes/Converted/; then
        error "Extraction failed. Check disk space or archive integrity."
    fi

    log "Unmounting volume..."
    hdiutil detach /Volumes/Converted/ -verbose
    ...
    if [[ $fin_sel == 0 ]]; then
        ...
        log "Converting to compressed DMG (Verbose)..."
        hdiutil convert "$TEMP_DMG" -format UDZO -o "$RESTORE_DMG" -verbose || error "Compression failed."
        log "Compression successful"

        log "Finalizing permissions (ASR Scan - Verbose)..."
        $sudo asr imagescan --source "$RESTORE_DMG" --verbose || error "ASR Scan failed."
...
dumper_modify_dmg() {
    ...
    local SELECTED_DMG
    SELECTED_DMG=$(osascript -e 'POSIX path of (choose file with prompt "Select DMG:")' 2>/dev/null)
    [[ -z "$SELECTED_DMG" ]] && return

    local DEST_DIR
    DEST_DIR=$(osascript -e 'POSIX path of (choose folder with prompt "Select Destination:")' 2>/dev/null)
    [[ -z "$DEST_DIR" ]] && return

    start_mac_authentication "authorize DMG modification"
    ...
    if [[ "$choice" == "2" ]]; then
        ...
        if [[ "$FORMAT" == "UDZO" ]]; then
            ...
            $sudo asr imagescan --source "$SELECTED_DMG" --verbose || error "ASR Scan failed."
    ...
    if [[ "$choice" == "2" ]]; then
        ...
        log "Converting to compressed DMG (Verbose)..."
        hdiutil convert "$SELECTED_DMG" -format UDZO -o "$FINAL_PATH" -verbose || error "Conversion failed."
        log "Compression successful"

        log "Finalizing permissions (ASR Scan - Verbose)..."
        $sudo asr imagescan --source "$FINAL_PATH" --verbose || error "ASR Scan failed."
        log "ASR ImageScan successful"
        ...
        log "Converting to modifiable format (Verbose)..."
        hdiutil convert "$SELECTED_DMG" -format UDRW -o "$FINAL_PATH" -verbose || error "Conversion failed."
        log "Decompression successful"

        local EXTRA_SPACE_STR
        EXTRA_SPACE_STR=$(get_gui_input "Extra Space for Modification (MB)?\n(Default is 0)" "0" "Resize DMG")
        local EXTRA_SPACE=${EXTRA_SPACE_STR:-0}

        if (( EXTRA_SPACE > 0 )); then
            log "Resizing DMG (+${EXTRA_SPACE}MB)..."
            hdiutil resize -size +"${EXTRA_SPACE}m" "$FINAL_PATH" -verbose || error "Resize failed."
...
dumper_initiate_dump() {
    ...
    if [[ "$FORMAT_CHOICE" == "1" ]]; then
         local EXTRA_SPACE_STR=$(get_gui_input "Extra Space for Modification (MB)?\n(Default is 0)" "0" "Resize DMG")
         EXTRA_SPACE=${EXTRA_SPACE_STR:-0}
    fi
    ...
    if [[ $AUTO_SUCCESS == 0 ]]; then
        ...
        if [[ $SSH_METHOD == 0 ]]; then
            ...
            # Manual Connection via GUI Prompts
            S_IP=$(get_gui_input "Enter Device IP Address:" "$S_IP" "SSH Configuration 1/4")
            if [[ -z "$S_IP" ]]; then return; fi
            S_PORT=$(get_gui_input "Enter SSH Port:" "$S_PORT" "SSH Configuration 2/4")
            if [[ -z "$S_PORT" ]]; then return; fi
            S_USER=$(get_gui_input "Enter SSH Username:" "$S_USER" "SSH Configuration 3/4")
            if [[ -z "$S_USER" ]]; then return; fi
            S_PASS=$(get_gui_input "Enter Device SSH Password:\n(Default is alpine. If you have changed it, please enter the correct one)" "$S_PASS" "SSH Configuration 4/4")
            if [[ -z "$S_PASS" ]]; then return; fi
        fi
    fi

    local TARGET_DIR
    TARGET_DIR=$(osascript -e 'POSIX path of (choose folder with prompt "Select where to save the DMG file:")' 2>/dev/null)
    if [[ -z "$TARGET_DIR" || ! -d "$TARGET_DIR" ]]; then
        warn "No valid save location selected. Operation cancelled."
        pause
    ...
    start_mac_authentication "authorize SSH connection"
    ...
    if [[ $FORMAT_CHOICE == 0 ]]; then
        ...
        log "Converting to compressed DMG (Verbose)..."
        hdiutil convert "$RAW_FILE" -format UDZO -o "$FINAL_PATH" -verbose || error "Compression failed."
        log "Compression successful"
...
ipsw_hwmodel_set() {
    ...
    case $device_type in
        ...
        iPhone10,[1245] ) hwmodel="iphone10";;
        iPhone10,[36]   ) hwmodel="iphone10b";;
        ...
        iPad6,1[12] ) hwmodel="ipad6e";;
        iPad7,[12]  ) hwmodel="ipad7";;
        iPad7,[34]  ) hwmodel="ipad7b";;
        iPad7,[56]  ) hwmodel="ipad7c";;
        iPad7,1[12] ) hwmodel="ipad7d";;
...
menu_advanced_options() {
    ...
    while [[ -z "$mode" && -z "$back" ]]; do
        ...
        if [[ $ctx == "powdersn0w"* || $ctx == "justboot" ]]; then
            ...
            menu_items+=("Set Custom Bootargs")
            if [[ -n $device_bootargs ]]; then
                menu_items+=("Clear Custom Bootargs")
            fi
        fi
        ...
        if [[ $ctx == "powdersn0w"* || $ctx == "justboot" ]]; then
            ...
            if [[ -n $device_bootargs ]]; then
                ...
                print "* Custom Bootargs:    [Not Selected]"
            fi
        fi
        echo
        ...
        case $selected in
            ...
            ;;
            "Set Custom Bootargs" )
                read -p "$(input 'Enter custom bootargs: ')" device_bootargs
            ;;
            "Clear Custom Bootargs" )
                device_bootargs=""
            ;;
            "Go Back" ) back=1;;
        esac
...
menu_miscutilities() {
    ...
    while [[ -z "$mode" && -z "$back" ]]; do
        ...
        if (( device_proc <= 10 )) && [[ $device_argmode == "none" ]]; then
...
menu_justboot() {
    ...
    while [[ -z "$mode" && -z "$back" ]]; do
        ...
        if [[ -n $device_bootargs ]]; then
            ...
            print "* You may enter custom bootargs (optional, experimental option) in Advanced Options"
...
device_enter_ramdisk() {
    if [[ $1 != "dumper" ]]; then
        # Check for A7, OR A8 devices that supported iOS 8 (iPhone 6/6+, iPad Air 2, iPod touch 6)
        if [[ $device_proc == 7 || ($device_proc == 8 && ($device_type == "iPhone7,"* || $device_type == "iPad5,3" || $device_type == "iPad5,4" || $device_type == "iPod7,1")) ]]; then
            input "Select SSH Ramdisk Version"
            print "* iOS 12 ramdisk is recommended for devices running iOS 9+."
            print "* iOS 8 ramdisk should be used for devices running iOS 7-8."
